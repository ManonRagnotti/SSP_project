...
export default({data, setRefresh}) => {
  ...
  const deleteData = async (_id) => {
    const res = await fetch('/api/hotel/' + _id, {method: 'DELETE'})
    await setRefresh(true)
  }

  /*
  * hotelCard : for each hotel, create a div with dynamic content
  */

  return (
    <div className="hotelcard">
    {
      (data && data.length) && data.map((h, index) => {
        return (<ul className="hotelInfo" key={index}>
          ...
          <li className="visite">{
              h.visite
                ? new Date(h.visite).toLocaleDateString()
                : ""
            }</li>
